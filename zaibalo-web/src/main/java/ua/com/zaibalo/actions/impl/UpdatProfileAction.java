...
public class UpdatProfileAction implements Action{
	...
	public void run(HttpServletRequest request, HttpServletResponse response, PrintWriter out) throws Exception {
		...
		for (FileItem item : fileItemsList) {
			...
			if (item.isFormField()) {
				...
				//AppConfigHelper.getConfig().getAppPropertyValue("");
		...
		String returnStatus = updateUserDisplayName(user, displayName, factory);
		status.append(returnStatus);
		
		returnStatus = updateUserPassword(user, newPassword, repeatPassword, factory);
		status.append(returnStatus);
		
		returnStatus = updateUserAbout(user, about, factory);
		status.append(returnStatus);
		
		returnStatus = updateUserNotifyOnPM(user, notifyOnPM, factory);
		status.append(returnStatus);
		
		String basePath = request.getServletContext().getRealPath("");
		returnStatus = updateUserAvatar(user, newAvatar, basePath, factory);
		status.append(returnStatus);
		
		request.setAttribute("update_status", status.toString());
		RequestDispatcher view = request.getRequestDispatcher("/secure/profileSettings.do");
		view.forward(request, response);
		
	}

	private String updateUserAbout(User user, String about, DataAccessFactory factory){
		...
		if(!about.equals(user.getAbout())){
			user.setAbout(about);
			factory.getUsersAccessInstance().updateUserAbout(user.getId(), about);
	...
	private String updateUserNotifyOnPM(User user, boolean notifyOnPM, DataAccessFactory factory){
		if(user.isNotifyOnPM() == notifyOnPM){
			return "";
		}else{
			user.setNotifyOnPM(notifyOnPM);
			factory.getUsersAccessInstance().updateUserNotifyOnPM(user.getId(), notifyOnPM);
			return StringHelper.getLocalString("email_notification_settings_changed");
		}
	}

	private String updateUserAvatar(User user, File savedFile, String path, DataAccessFactory factory) throws IOException{
		if(savedFile == null){
			return "";
		}
		...
		factory.getUsersAccessInstance().updateUserImage(user.getId(), userId  + "." + ext, userId + ".thumbnail" + "." + ext);
		
		return StringHelper.getLocalString("user_image_successfully_updated");
	}
	
	private String updateUserPassword(User user, String newPassword, String repeatPassword, DataAccessFactory factory) throws NoSuchAlgorithmException {
	...
	private String updateUserDisplayName(User user, String value, DataAccessFactory factory){
		...
		if(!value.equals(user.getDisplayName())){
			...
			if(factory.getUsersAccessInstance().getUserByDisplayName(value) != null){
				return StringHelper.getLocalString("user_display_name_already_exists", value);
			}
			
			user.setDisplayName(value);
			factory.getUsersAccessInstance().updateUserDisplayName(user.getId(), value);
